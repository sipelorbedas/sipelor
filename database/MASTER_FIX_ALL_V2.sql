-- ============================================
-- MASTER FIX V2: Complete solution for f.name error
-- ============================================
-- This script fixes EVERYTHING with correct notifications schema:
-- 1. RLS Policies
-- 2. Trigger Functions (using correct columns: body, not message)
-- 3. Triggers
--
...
BEGIN
    RAISE NOTICE '╔════════════════════════════════════════════════════════════╗';
    RAISE NOTICE '║           🚀 MASTER FIX SCRIPT V2 STARTING                 ║';
    RAISE NOTICE '╠════════════════════════════════════════════════════════════╣';
    RAISE NOTICE '║ This will fix:                                            ║';
    RAISE NOTICE '║ 1. RLS Policies                                           ║';
    RAISE NOTICE '║ 2. Trigger Functions (correct schema)                     ║';
...
-- Notifications table schema: user_id, type, title, body, data, read, created_at
...
BEGIN
    IF (TG_OP = 'UPDATE' AND OLD.payment_status != NEW.payment_status AND NEW.payment_status IN ('verified', 'rejected')) THEN
        ...
        SELECT 
            NEW.user_id,
            CASE 
                WHEN NEW.payment_status = 'verified' THEN 'payment_verified'
                WHEN NEW.payment_status = 'rejected' THEN 'payment_rejected'
                ELSE 'payment'
            END,
            CASE 
            ...
            CASE 
                ...
                WHEN NEW.payment_status = 'rejected' THEN 'Pembayaran untuk ' || COALESCE(f.venue_name, 'lapangan') || ' ditolak. Silakan upload ulang.'
...
DO $$ 
BEGIN
    RAISE NOTICE '✅ Part 2 Complete: Trigger functions recreated with correct schema';
    RAISE NOTICE '   - Using f.venue_name instead of f.name';
    RAISE NOTICE '   - Using body instead of message';
    RAISE NOTICE '   - Using correct notifications table columns';
END $$;
...
BEGIN
    ...
    RAISE NOTICE '║    - message → body                                       ║';
    RAISE NOTICE '║    - Removed non-existent columns                         ║';
