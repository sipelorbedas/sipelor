/**
 * ╔══════════════════════════════════════════════════════════════════════╗
 * ║  SIPELOR BEDAS — Buat Akun Auth OPD / Pimpinan (CARA BENAR)        ║
 * ║                                                                      ║
 * ║  KENAPA PAKAI SCRIPT INI, BUKAN SQL?                                ║
 * ║  SQL INSERT langsung ke auth.users menggunakan pgcrypto crypt()     ║
 * ║  yang menghasilkan hash bcrypt format $2a$ (OpenBSD).               ║
 * ║  GoTrue (Supabase Auth) menggunakan Go bcrypt ($2b$) dan            ║
 * ║  verifikasi hash $2a$ dari pgcrypto TIDAK reliable di Supabase      ║
 * ║  hosted → password selalu ditolak (Invalid login credentials).      ║
 * ║                                                                      ║
 * ║  Script ini menggunakan auth.admin.createUser() → password          ║
 * ║  di-hash langsung oleh GoTrue → PASTI bisa login.                  ║
 * ║                                                                      ║
 * ║  CARA PAKAI:                                                         ║
 * ║    cd database                                                       ║
...
// Daftar OPD: [username, nama_lengkap, email_login]
// Email login diambil dari opd_organizations.email agar konsisten dengan database.
// Jika email kosong, fallback ke username@sipelor.go.id
const OPD = [
  // BADAN
...
async function run() {
  ...
  const chk = await sb.auth.admin.listUsers({ perPage: 1 });
  if (chk.error) {
    console.error("❌ ERROR koneksi:", chk.error.message);
    console.error("   Pastikan SERVICE_ROLE_KEY benar (bukan anon key!)");
    process.exit(1);
  }
  console.log("✅ Koneksi OK.\n");
  console.log(`🚀 Membuat ${OPD.length} akun OPD / Pimpinan...`);
  console.log("─".repeat(70));

  let ok = 0, skip = 0, fail = 0;

  for (const [uname, name, email] of OPD) {
    process.stdout.write(`→ ${uname.padEnd(20)} ${email.padEnd(38)} `);

    // Buat user via Admin API (password di-hash oleh GoTrue → pasti bisa login)
    ...
    if (error) {
      if (error.message.toLowerCase().includes("already") ||
          ...
          error.message.toLowerCase().includes("email address has already")) {
        // Akun sudah ada — pastikan profile username ter-set
        const { data: existProf } = await sb
          .from("profiles").select("id, username").eq("email", email).maybeSingle();
        if (existProf && !existProf.username) {
    ...
    // Insert / update profile dengan username ter-set
