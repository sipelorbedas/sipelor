...
async function renderOPD(api, container) {
  ...
  <div class="page-header">
    <div class="page-header-left">
      ...
      <div class="page-subtitle">Kelola daftar OPD, kredensial login, dan blokir jadwal lapangan untuk keperluan resmi.</div>
    </div>
  </div>

  <!-- ── Daftar OPD (full width) ── -->
  <div class="card" style="padding:0;overflow:hidden;margin-bottom:20px;">
    <div style="padding:16px 20px;border-bottom:1px solid var(--border);display:flex;align-items:center;justify-content:space-between;">
      <div>
        <div style="font-size:15px;font-weight:700;color:var(--text);">Daftar OPD & Pimpinan</div>
        <div style="font-size:12px;color:var(--text-muted);margin-top:2px;">Kelola organisasi, diskon, dan kredensial login</div>
      </div>
  ...
  <!-- ── Form Blokir Jadwal (full width) ── -->
  <div class="card" style="padding:28px 32px;margin-bottom:20px;max-width:680px;">
  ...
  <!-- Modal Lihat Kredensial -->
  <div class="modal-overlay" id="opd-cred-modal">
    <div class="modal" style="max-width:420px;">
      <div class="modal-header">
        <div class="modal-title">🔑 Kredensial Login</div>
        <button class="btn btn-ghost btn-icon" onclick="closeModal('opd-cred-modal')">✕</button>
      </div>
      <div class="modal-body" id="opd-cred-modal-body"></div>
    </div>
  </div>`;
...
window._onOPDSelectChange = function() {
  ...
  if (badge && label) {
    if (pct > 0 && opt?.value) {
      label.textContent = `Diskon ${pct}% untuk ${opt.text.split(' (Diskon')[0]}`;
      badge.style.display = '';
    } else {
...
// ── Tampilkan password terenkripsi (masked) dengan tombol copy ─
function _credentialHtml(username, password) {
  const safeUser = username || '—';
  const hasUser  = !!username;
  return `
  <div style="display:flex;flex-direction:column;gap:4px;">
    <div style="display:flex;align-items:center;gap:6px;">
      <span style="font-family:monospace;font-size:12px;font-weight:700;color:var(--primary,#A855F7);">${safeUser}</span>
      ${hasUser ? `<button
        style="background:none;border:none;cursor:pointer;padding:0;color:var(--text-muted);line-height:1;"
        title="Lihat detail kredensial"
        onclick="showOPDCredential('${safeUser}', '${(password||'Sipelor@2026').replace(/'/g,"\\'")}')"
      >🔑</button>` : ''}
    </div>
    ${hasUser ? `<span style="font-size:11px;color:var(--text-muted);font-family:monospace;">••••••••••</span>` : ''}
  </div>`;
}
...
function _renderOPDList() {
  ...
  <table style="min-width:980px;">
    <thead>
      <tr>
        <th style="min-width:180px;">Nama OPD / Pimpinan</th>
        <th style="min-width:140px;">Kredensial Login</th>
        <th style="min-width:110px;">PIC</th>
        <th style="min-width:170px;">Kontak</th>
        <th style="min-width:80px;text-align:center;">Diskon</th>
    ...
    <tbody>
      ...
      <tr>
        <td style="font-weight:600;font-size:13px;">
          <div style="word-break:break-word;">${o.name}</div>
        </td>
        <td>${_credentialHtml(o.username, o.default_password)}</td>
        ...
        <td style="text-align:center;">${_discountBadgeHtml(o.discount_percentage)}</td>
        <td>
...
// ── Modal lihat detail kredensial OPD ──────────────────────────
window.showOPDCredential = function(username, password) {
  const loginEmail = username.includes('@') ? username : `${username}@sipelor.go.id`;
  document.getElementById('opd-cred-modal-body').innerHTML = `
  <div style="display:flex;flex-direction:column;gap:16px;">
    ...
    ">
      <div style="font-size:12px;color:var(--text-muted);margin-bottom:12px;text-transform:uppercase;letter-spacing:.5px;font-weight:700;">
        🔑 Info Login Aplikasi
      </div>
      ...
      <div style="margin-bottom:10px;">
        ...
        <div style="display:flex;align-items:center;gap:8px;">
          <code style="flex:1;background:rgba(0,0,0,0.25);padding:8px 12px;border-radius:8px;font-size:14px;font-weight:700;color:#A855F7;">${username}</code>
          <button class="btn btn-outline btn-sm" onclick="navigator.clipboard.writeText('${username}');showToast('Username disalin!','success',1800);">📋</button>
        </div>
      </div>

      <div style="margin-bottom:10px;">
        <div style="font-size:11px;color:var(--text-muted);margin-bottom:4px;">Email Login</div>
        <div style="display:flex;align-items:center;gap:8px;">
          <code style="flex:1;background:rgba(0,0,0,0.25);padding:8px 12px;border-radius:8px;font-size:13px;color:var(--text);">${loginEmail}</code>
          <button class="btn btn-outline btn-sm" onclick="navigator.clipboard.writeText('${loginEmail}');showToast('Email disalin!','success',1800);">📋</button>
        </div>
      </div>

      <div>
        <div style="font-size:11px;color:var(--text-muted);margin-bottom:4px;">Password Default</div>
        <div style="display:flex;align-items:center;gap:8px;">
          <code id="cred-pw-display" style="flex:1;background:rgba(0,0,0,0.25);padding:8px 12px;border-radius:8px;font-size:14px;font-weight:700;color:#10B981;letter-spacing:2px;">••••••••••</code>
          <button class="btn btn-outline btn-sm" onclick="toggleCredPw('${password}')">👁️</button>
          <button class="btn btn-outline btn-sm" onclick="navigator.clipboard.writeText('${password}');showToast('Password disalin!','success',1800);">📋</button>
        </div>
      </div>
    </div>

    <div style="background:rgba(245,158,11,0.10);border:1px solid rgba(245,158,11,0.30);border-radius:10px;padding:12px 14px;">
      <div style="font-size:12px;color:#F59E0B;font-weight:700;">⚠️ Keamanan</div>
      <div style="font-size:12px;color:var(--text-muted);margin-top:4px;line-height:1.6;">
        Minta pengguna mengganti password setelah login pertama.<br/>
        Jangan bagikan kredensial melalui saluran tidak aman.
      </div>
    </div>

    <div class="modal-footer" style="padding:0;">
      <button class="btn btn-outline" onclick="closeModal('opd-cred-modal')">Tutup</button>
    </div>
  </div>`;
  openModal('opd-cred-modal');
};

// Toggle show/hide password di modal kredensial
window.toggleCredPw = function(pw) {
  const el = document.getElementById('cred-pw-display');
  if (!el) return;
...
async function loadOPDBookingsTable(api, page = _opdBookingPage) {
  ...
  wrap.innerHTML = `
  <div style="overflow-x:auto;max-height:380px;overflow-y:auto;border-radius:8px;">
  <table>
    <thead>
      <tr>
        <th>ID</th>
...
window.openOPDModal = function(opd = null) {
  ...
  const defaultPw  = opd?.default_password || 'Sipelor@2026';
  ...
  document.getElementById('opd-modal-body').innerHTML = `
    ...
    <div class="form-group">
      ...
      <input type="text" class="form-control" id="opd-name" value="${opd?.name || ''}"
        placeholder="Contoh: Dinas Pendidikan Kab. Bandung" required maxlength="160" />
    </div>

    <!-- PIC -->
    <div class="form-group">
      <label class="form-label">Nama PIC / Pimpinan</label>
      <input type="text" class="form-control" id="opd-pic" value="${opd?.contact_person || ''}"
        placeholder="Contoh: Kepala Dinas / Camat" maxlength="80" />
    </div>
    ...
    <div style="
      ...
      margin-bottom:16px;
    ">
      <div style="font-size:13px;font-weight:700;color:var(--primary,#A855F7);margin-bottom:12px;">🔑 Kredensial Login Aplikasi</div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
        <div class="form-group" style="margin-bottom:0;">
          <label class="form-label">Username</label>
          <input type="text" class="form-control" id="opd-username" value="${opd?.username || ''}"
            placeholder="Contoh: dispora / kec.soreang"
            style="font-family:monospace;font-weight:600;color:var(--primary,#A855F7);"
            maxlength="60" />
          <div style="font-size:10px;color:var(--text-muted);margin-top:4px;">Huruf kecil, tanpa spasi</div>
        </div>
        <div class="form-group" style="margin-bottom:0;">
          <label class="form-label">Password Default</label>
          <div style="position:relative;">
            <input type="password" class="form-control" id="opd-password" value="${defaultPw}"
              placeholder="Sipelor@2026"
              style="font-family:monospace;padding-right:42px;"
              maxlength="80" />
            <button type="button"
              style="position:absolute;right:10px;top:50%;transform:translateY(-50%);background:none;border:none;cursor:pointer;color:var(--text-muted);font-size:15px;"
              onclick="_toggleOpdPwField()"
              title="Tampilkan/Sembunyikan">👁️</button>
          </div>
          <div style="font-size:10px;color:var(--text-muted);margin-top:4px;">Default: Sipelor@2026</div>
        </div>
      </div>
    </div>
...
// Toggle show/hide password field di modal
window._toggleOpdPwField = function() {
  const el = document.getElementById('opd-password');
  if (!el) return;
...
window.saveOPD = async function(id) {
  ...
  const usernameRaw = (document.getElementById('opd-username')?.value || '').trim().toLowerCase().replace(/\s+/g,'');
  ...
  const payload = {
    ...
    default_password:    document.getElementById('opd-password').value.trim() || 'Sipelor@2026',
    discount_percentage: discountPct,
