  ...
  try {
    ...
    container.innerHTML = `
      <div class="proof-empty-state">
        ...
        <div style="margin-top:8px;font-size:13px;color:var(--text-muted);">Gagal memuat bukti pembayaran.</div>
      </div>`;
  }
}
...
window.verifyProofStatus = async function(proofId, bookingId, containerId) {
  ...
  try {
    // Update the proof record status → approved
    await window._bookingAPI.updateProofStatus(proofId, 'verified');
    // Also sync the booking-level payment_status → verified
    await window._bookingAPI.updatePaymentStatus(bookingId, 'verified');
